...
void	draw_texture_line(mlx_image_t *img, t_ray ray, t_texture_line t)
{
	...
	while (t.texture_coor.y < t.screen_wall_height)
	{
		if (t.screen_coor.y > HEIGHT)
			break ;
		pixel_col = color_picker(t.texture_coor, t.texture,
				ray.darkener_percent);
		mlx_put_pixel(img, t.screen_coor.x, t.screen_coor.y, pixel_col);
		t.screen_coor.y++;
...
void	to_3d(t_ray ray, t_game *game, float ray_index)
{
	...
	rounded.x = roundf(ray.colision_point.x);
	rounded.y = roundf(ray.colision_point.y);
	ray.darkener_percent = 4 / ray.colision_len;
	if (ray.darkener_percent > 1)
		ray.darkener_percent = 1;
	check_east_west(ray, game, ray_index, rounded);
	check_north_south(ray, game, ray_index, rounded);
}
